...
export function createSpin(number, wheelStart, ballStart, front = 0.15) {
  ...
  const captureWheel = wheelStart + (wheelEnd-wheelStart)*ease(.84);
...
export function sampleSpin(spin, progress) {
  ...
  const drop = smooth((t-.59)/.25);
  const settling = smooth((t-.84)/.16);
  let angle = spin.ballStart + (spin.ballEnd-spin.ballStart)*ease(Math.min(1,t/.84));
  if (t >= .84) angle = spin.ballEnd + wheel-spin.captureWheel + Math.sin((t-.84)*110)*.023*(1-settling);
  const bounce = t > .68 && t < 1 ? Math.abs(Math.sin((t-.68)*80))*.16*Math.sin(Math.PI*clamp((t-.68)/.32)) : 0;
